    ...
    protected void survey(int households[]) {
        for (int i = row -1; i <= row + 1; i++) {
            for (int j = column - 1; j <= column + 1; j++) {
               ...
               if (i < 0 || j < 0 || i >= neighborhoodGrid.getSize() || j >= neighborhoodGrid.getSize()) {
                    continue; // skips items out of bounds

                } else {
                    Household neighbor = neighborhoodGrid.grid[i][j];
                    households[neighbor.getPreference().ordinal()]++;
